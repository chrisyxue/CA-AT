...
from Continual.EWC import EWC, update_ewc_params_importance
...
# dataset:
parser.add_argument('--dataset', '--ds', default='cifar100', choices=['cifar10', 'svhn', 'stl10','cifar100','mnist'], help='which dataset to use')
...
parser.add_argument('--method', default='Finetune',choices=['Finetune','LwF','MAS','EWC']) # rehersal methods

# For LwF
parser.add_argument('--tempreture',default=2,type=float) # the number of tasks
parser.add_argument('--alpha', default=4,type=float) # rehersal methods

# For EWC
parser.add_argument('--ewc_lambda',default=10,type=float)
parser.add_argument('--ewc_mode',default='separate',choices=['separate','online'])
parser.add_argument('--ewc_decay_factor',default=0.1,type=float)
...
def init_model(args):
    ...
    if args.dataset == 'mnist':
        if args.model == 'ResNet34':
            ...
            enc = enc_fn().cuda()
            cls_fn = ResNet_CLS
            classifier = cls_fn(num_classes=int(10/args.task_num)).cuda()
            model = ComModel(enc,classifier)
        elif args.model == 'SimpleCNN':
            enc = SimpleCNN_ENC().cuda()
            classifier = SimpleCNN_CLS(num_classes=int(10/args.task_num)).cuda()
            model = ComModel(enc,classifier)
...
if args.dataset == 'cifar100':
    ...
    dataset = SplitCIFAR100(n_experiences=args.task_num,seed=1234,
                            return_task_id=True,
    ...
    dataset = SplitMNIST(n_experiences=args.task_num,
    seed=1234,
    return_task_id=True,
    dataset_root='/localscratch2/xuezhiyu/datasets/mnist',
    train_transform=train_transform,eval_transform=test_transform
    )
...
save_folder = os.path.join('/localscratch2/xuezhiyu/Adver_Continual_Regular', args.dataset, args.method, 'Task_Num_'+str(args.task_num),model_str, '%s_%s' % (attack_str, loss_str))

if args.method == 'EWC':
    save_folder = os.path.join('/localscratch2/xuezhiyu/Adver_Continual_Regular', args.dataset, args.method, args.ewc_mode+'_'+str(args.ewc_lambda),'Task_Num_'+str(args.task_num),model_str, '%s_%s' % (attack_str, loss_str))

if args.method == 'LwF':
    save_folder = os.path.join('/localscratch2/xuezhiyu/Adver_Continual_Regular', args.dataset, args.method, 'alpha_'+str(args.alpha),'Task_Num_'+str(args.task_num),model_str, '%s_%s' % (attack_str, loss_str))
...
for experience in train_stream:
    ...
    for epoch in range(start_epoch, args.epochs):
        ...
        if args.method == 'Finetune':
            train_loader, model, attacker, optimizer, args, scheduler, accs, accs_adv, losses = Finetune(train_loader, model, attacker, optimizer, args, scheduler, accs, accs_adv, losses)
        elif args.method == 'LwF':
            train_loader,model,attacker,optimizer,args,scheduler,accs,accs_adv,losses,prev_model = LwF(train_loader, model, attacker, optimizer, args, scheduler, accs, accs_adv, losses, prev_model)
        elif args.method == 'EWC':
            exp_counter = experience.task_label
            EWC(train_loader, model, attacker, optimizer, args, scheduler, accs, accs_adv, losses, saved_params, importances_list, exp_counter)
            # pdb.set_trace()
        ...
        if accs.avg > best_TA:
            # best_TA = accs.avg
            torch.save(model.enc.state_dict(),os.path.join(save_folder_models,'Task_'+str(current_task)+'_enc_best.pth'))
            torch.save(model.cls.state_dict(),os.path.join(save_folder_models,'Task_'+str(current_task)+'_cls_best.pth'))
        # if args.Lambda != 0:
   ...
   #  eval_this_epoch = (epoch % 20 == 0) or (epoch>=int(0.7*args.epochs))
    ...
    for val_experience in test_stream[:current_task+1]: 
        model.eval()
        requires_grad_(model, False)
        print('Task {}'.format(val_experience.task_label)) 
        print('Classes in this task:', val_experience.classes_in_this_experience)   
        val_accs, val_accs_adv = AverageMeter(), AverageMeter()
        task_id_val = val_experience.current_experience
        

        model = init_model(args)
        # shared parameters
        model.enc.load_state_dict(torch.load(os.path.join(save_folder_models,'Task_'+str(current_task)+'_enc_best.pth')))
        # task specific parameters
        model.cls.load_state_dict(torch.load(os.path.join(save_folder_models,'Task_'+str(task_id_val)+'_cls_best.pth')))

        # pdb.set_trace()

        current_test_set = val_experience.dataset


        test_loader = DataLoader(current_test_set, batch_size=args.batch_size, shuffle=False, num_workers=3,
                                drop_last=False, pin_memory=True)
    ...
    if args.method == 'LwF':
        ...
        update_ewc_params_importance(experience, model, criterion, optimizer, args.batch_size, saved_params, keep_importance_data, exp_counter, args.ewc_decay_factor, importances_list, args.ewc_mode)
