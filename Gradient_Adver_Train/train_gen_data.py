...
parser.add_argument('--epochs', '-e', default=400, type=int, help='number of total epochs to run')
...
for epoch in range(start_epoch, args.epochs):
    save_this_epoch = (epoch % 10 == 0) and (epoch>150)
    print('save_this_epoch',save_this_epoch)

    if save_this_epoch:
        save_folder_models_epoch = os.path.join(save_folder_models,'epoch_'+str(epoch))
        save_folder_data_epoch = os.path.join(save_folder_data,'epoch_'+str(epoch))
        create_dir(save_folder_models_epoch)
        create_dir(save_folder_data_epoch)
    ...
    if save_this_epoch:
        torch.save(model.state_dict(), os.path.join(save_folder_models_epoch, str(epoch)+'.pth')) 
