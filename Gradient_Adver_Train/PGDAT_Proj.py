...
from attacks.pdg_grad_proj import gradient_adv_train,gradient_adv_train_cat
...
# Use Curriculumn AT
parser.add_argument('--use_cat',action='store_true',help='use CAT or not')
parser.add_argument('--expect_acc_cat', default=0.75, type=float, help='update k when testacc exceed this')
parser.add_argument('--k_cat', default=-1, type=int, help='initial k for cat')
parser.add_argument('--maxk_cat', default=20, type=int, help='max k')
...
if args.grad_norm is False:
    attack_str += '_NoGradNorm_'
if args.use_trades is True:
    attack_str += '-TRADES'
if args.use_pretrain is True:
    attack_str += '-Pretrain-'
if args.use_cat is True:
    attack_str += '-UseCat-' + str(args.k_cat) + '-' + str(args.maxk_cat)

loss_str = 'lambda%s' % (args.Lambda)
if args.use_cat is True:
    save_folder = os.path.join('/root/results/Gradient_Adver_Train', args.dataset, model_str, '%s_%s_%s_%s' % (attack_str, opt_str, decay_str, loss_str))
else:
...
# use pretraining
if args.use_pretrain is True:
    pretrain_file = '/root/results/Gradient_Adver_Train/cifar10/ResNet34/untargeted-pgd-8-7_e200-b256_sgd-lr0.1-m0.9-wd0.0005_cos_lambda0.0/latest.pth'
...
for epoch in range(start_epoch, args.epochs):
    ...
    for i, (imgs, labels) in enumerate(train_loader):
        ...
        if args.use_cat is True:
            model,optimizer = gradient_adv_train_cat(args,model, criterion, optimizer, imgs, imgs_adv, labels, epoch)
        else:
    ...
    # Update K for CAT
    if args.use_cat:
        if args.k_cat < args.maxk_cat and i % 10 == 0:
            k = k + 1 
    ...
    if args.dataset == 'cifar10' or 'cifar100':
        ...
        eval_this_epoch =  (epoch % 40 == 0) or (epoch>=int(0.9*args.epochs)) # bool
