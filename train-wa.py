...
from core.utils import render_resfile_name

from gowal21uncovering.utils import WATrainer

# Setup
parse = parser_train()
parse.add_argument('--tau', type=float, default=0.995, help='Parameters for Weight averaging decay.')
args = parse.parse_args()

# render dir
args.log_dir = render_resfile_name(args=args)
...
# Load data
seed(args.seed)
train_dataset, test_dataset, train_dataloader, test_dataloader = load_data(args,
    DATA_DIR, BATCH_SIZE, BATCH_SIZE_VALIDATION, use_augmentation=args.augment, shuffle_train=True)
del train_dataset, test_dataset

# Adversarial Training
seed(args.seed)
args.tau = 0 # we don't use WA
if args.tau:
    print ('Using WA.')
...
if NUM_ADV_EPOCHS > 0:
    ...
    # logger.log('Standard Train Accuracy-\tTest: {:2f}%.'.format(trainer.eval(train_dataloader)*100))
...
for epoch in range(1, NUM_ADV_EPOCHS+1):
    ...
    logger.log('Clean Loss: {:.4f}.\tLR: {:.4f}'.format(res['clean_loss'], last_lr))
    logger.log('Adver Loss: {:.4f}.\tLR: {:.4f}'.format(res['adv_loss'], last_lr))
    ...
    epoch_metrics.update({'epoch': epoch, 'lr': last_lr, 'test_clean_acc': test_acc, 'test_adv_acc': ''})
    
    if epoch % args.adv_eval_freq == 0 or epoch == NUM_ADV_EPOCHS:        
        test_adv_acc = trainer.eval(test_dataloader, adversarial=True)
        logger.log('Adversarial Accuracy-\tTrain: {:.2f}%.\tTest: {:.2f}%.'.format(res['adv_acc']*100, 
                                                                                   test_adv_acc*100))
        epoch_metrics.update({'test_adversarial_acc': test_adv_acc})
    else:
        logger.log('Adversarial Accuracy-\tTrain: {:.2f}%.'.format(res['adv_acc']*100))
