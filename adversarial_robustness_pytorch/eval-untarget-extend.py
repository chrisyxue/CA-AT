...
from attack_test import eval_untarget_adv_test
...
_, _, train_dataloader, test_dataloader = load_data(DATA_DIR, BATCH_SIZE, BATCH_SIZE_VALIDATION, use_augmentation=False, 
                                                    shuffle_train=False,num_workers=1)
...
# eps_inf_lst = [8/255]
# eps_l2_lst = [0.5]
eps_inf_lst = [8/255,12/255,16/255,20/255,24/255,28/255,32/255,36/255,40/255]
eps_l2_lst = [0.5,0.75,0.1,1.25,1.5,1.75,2,2.25,2.5]
assert(len(eps_inf_lst) == len(eps_l2_lst))
res_lst = []
for i in range(len(eps_inf_lst)):
    eps_inf = eps_inf_lst[i]
    eps_l2 = eps_l2_lst[i]
    log_path = LOG_DIR + '/log-untarget-'+str(int(eps_inf*255))+'.log'
    res = eval_untarget_adv_test(model, test_dataloader, eps_inf, eps_l2)
    res_lst.append(res)
    res_df = pd.DataFrame(res_lst)
    res_df.to_csv(os.path.join(LOG_DIR,'untarget-attacks-eval.csv'))
